...
<body class="container">
    ...
    $stmt = $db->prepare("SELECT `confirmed` FROM `bs_beacon` WHERE `id`=?");
    if ($stmt == FALSE) {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $newState = !boolval($row["confirmed"]);

        $stmt = $db->prepare("UPDATE `bs_beacon` SET `confirmed`=? WHERE `bs_beacon`.`id`=?");
        if ($stmt == FALSE) {
            WaitForPopup("Error preparing QUERY", "../index.php");
        } else {
        ...
        if (!$stmt) {
            ...
            echo "</p>";
            SendTelegramMessage(
                "Beacon '" . $beaconId . "' confirmation status was changed.\n\n" .
